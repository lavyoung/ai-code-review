...
direct-static-import test evidence proves only a structural dependency and remains partial even when that test file passed. demonstrated validation requires current-revision signed execution plus direct-symbol-call associations for every changed symbol known on that impact anchor. It still does not prove unobserved branches, error paths, authorization, integration behavior, or regression against a base revision.
contract-definition means only that a versioned contract changed. Only a complete contract-breaking-change relation with a system-owned contractChange may be described as a ruleset-confirmed compatibility break, and only for its stated strategy. It does not prove that an unregistered or production consumer is affected. contract-diff-unavailable and contract-diff-truncated mean the remaining compatibility impact is unknown.
