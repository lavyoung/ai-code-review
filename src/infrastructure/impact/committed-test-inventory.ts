...
import {extractTestReferences, type SupportedTestFramework} from "./test-reference-extractor.js";
...
const classifyFramework = (content: string): SupportedTestFramework | undefined => {
...
/**
 * 仅读取同一 HEAD commit 中受限数量的候选测试文件，生成无测试路径、无正文的清单。
 *
...
export class CommittedTestInventory implements TestInventoryPort {
    ...
    public async discover(signal: AbortSignal): Promise<TestInventorySummary> {
        const {stdout: revisionOutput} = await execFileAsync("git", ["rev-parse", "--verify", "HEAD^{commit}"], {
            cwd: this.workingDirectory,
            encoding: "utf8",
            signal,
        });
        const sourceRevision = revisionOutput.trim().toLowerCase();
        if (!/^[a-f0-9]{40}$/u.test(sourceRevision)) {
            throw new Error("Committed test inventory revision was invalid.");
        }
        const {stdout} = await execFileAsync("git", ["ls-tree", "-r", "--name-only", "-z", sourceRevision], {
            cwd: this.workingDirectory,
            encoding: "utf8",
            maxBuffer: 256 * 1024,
            signal,
        ...
        const frameworks = new Set<SupportedTestFramework>();
        ...
        for (const path of paths) {
            ...
            const {stdout: content} = await execFileAsync("git", ["show", "--no-textconv", `${sourceRevision}:${path}`], {
                cwd: this.workingDirectory,
                encoding: "utf8",
                maxBuffer: 256 * 1024,
                signal,
            ...
            if (framework !== undefined) {
                ...
                assetCount += 1;
                references.push(...extractTestReferences(path, content, framework, sourceRevision));
            }
        }
        return {
