...
} from "./signed-sandbox-test-report.js";

/** 受控测试运行器产出的签名结果文件配置；其路径与签名密钥都禁止记录。 */
export type SandboxedTestResultAnalyzerConfiguration = SignedSandboxTestReportConfiguration;
...
export class SandboxedTestResultAnalyzer implements ReviewAnalyzer {
    ...
    public async analyze(request: AnalysisRequest): Promise<ReviewAnalysis> {
        const report = await new SignedSandboxTestReportReader(this.configuration, this.revisionProvider)
            .read(request.signal);
