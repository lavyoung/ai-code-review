...
数字、布尔和 `null` 字面量可在同参数数量的重载中作受控筛选；变量、泛型推断或多个仍可行候选不会强行分派。索引支持一跳且目标明确的
barrel `export`/`export *`，包括两侧别名；多跳、循环或多个同名来源保持未知。Java 显式通配符 import 仅在目标候选唯一时参与实例、静态、
继承和实现解析。
...
对于相对 TypeScript/CommonJS 引用及 Java import，系统可将已发现测试静态引用与本次变更源码作精确匹配。这只产生
