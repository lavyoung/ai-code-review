...
Java 语法树确认，再使用显式类型 import、同包类型和字段/参数声明解析静态或实例接收者。重载仅在参数个数能唯一匹配时建立调用边，
同参数个数但类型不同等无法可靠分派的情况保持 `overload-resolution-unavailable`。继承和实现目标可通过显式 import 或同包类型解析，
泛型参数不会被误当成独立父类型。
