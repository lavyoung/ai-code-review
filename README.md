...
每次评审会从已提交 diff 的 TypeScript/Java 变更声明建立带签名和源码摘要的 `SymbolIdentity`，保守关联 base/head 间的重命名、
移动、重载变化和实现替换，并提取新增静态 `import`、CommonJS `require`、调用、实现/继承、配置、事件及持久化写入关系。构建后仅将
锚点、安全目标标识、符号摘要与明确限制传给 AI；原始关联文件正文不会传出。索引会在文件数、单文件大小、总字符数和并发上限内
读取 Git 对象数据库中的实际 base/head 源码，不读取工作区文件；因此方法体变更可关联到所属符号，带明确 TypeScript import 或 Java
类型 import 的未修改调用方也可形成跨文件调用边。未解析的调用目标仍保持 `unknown`，这不是完整仓库调用图。动态 import、动态分派、
反射、代码生成、歧义或未匹配符号、未支持语言、快照不可用及扫描截断都会明确降级，
不代表“没有影响”，也不能单独生成已确认缺陷、缺失测试或质量门禁。

跨文件解析使用 TypeScript AST 识别默认、具名及 namespace import，其中具名和 namespace 别名可回到原始导出符号；Java 调用先由
Java 语法树确认，再使用显式类型 import、同包类型和字段/参数声明解析静态或实例接收者。重载先按参数个数筛选，再仅使用可确定的字面量
类型消除同数量候选；变量或多个仍可行候选保持 `overload-resolution-unavailable`。继承和实现目标可通过显式 import 或同包类型解析，
泛型参数不会被误当成独立父类型。

完整 TypeScript 快照使用 AST 建立 `模块.类#方法` 身份，因此显式导入类的参数、字段或 `new` 实例调用可以关联到被修改方法。字符串、
数字、布尔和 `null` 字面量，以及具有明确基础类型注解或常量初始化的局部标识符，可在同参数数量的重载中作受控筛选；泛型推断或多个仍
可行候选不会强行分派。索引在四层深度内解析目标明确的 barrel `export`/`export *` 及两侧别名，并检测循环；超深、循环或多个同名来源
...
对于相对 TypeScript/CommonJS 引用及 Java import，系统可将已发现测试静态引用与本次变更源码作精确匹配。普通
`direct-static-import` 只证明结构依赖，即使对应测试文件在签名报告中通过，覆盖状态仍最高为 `partial`。Vitest/Jest 使用纯内存
TypeScript 符号解析确认测试回调内的直接调用确实绑定到 import；JUnit 使用 Java 语法树识别 `@Test` 方法内显式导入类型的直接调用。只有同一影响锚点上系统已知的
全部变更符号均有 `direct-symbol-call` 关联，并且同一提交的签名报告证明相关测试通过时，普通行为义务才可标为 `demonstrated`。
测试资产与关联会记录不透明测试 ID、框架、提交 SHA 和关联方式；敏感测试文件及测试所引用的敏感目标路径都会在本地边界剔除。测试名称
相似、目录相邻和动态依赖均不参与匹配。

对于新增或修改的 TypeScript/Java 源码，系统会额外建立一次已锚定的源码变更关系，因此即使源码本身没有新增 import，引用该源码的
已发现测试也可以形成上述有限关联。调用和技术边仍是观察性证据，不是完整调用图、控制流分析或行为覆盖证明；无法锚定或解析的源码
改动保持 `unknown`。跨文件调用关系只用于观察和解释，不会形成确定性发现或门禁依据。

如启用 `sandbox_tests`，外部受控沙箱可在既有签名结果中附加 `passedTests`（每项只含测试文件路径）。只有 HMAC 签名有效、
`sourceRevision` 等于测试清单的当前 `HEAD`，且该测试满足上述完整直接符号关联时，影响覆盖才会标为 `demonstrated`。这仍不证明未观测
分支、错误路径、权限、集成行为或相对 base revision 的回归。旧报告没有 `passedTests` 时仍可报告已锚定的失败，但不会产生通过或覆盖证明；
CLI 自身不会执行任何仓库测试脚本。
