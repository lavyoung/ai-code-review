...
- 每个 `AnalyzerRun` 有独立超时、重试、错误分类和脱敏诊断信息；降级运行只输出固定原因码，不输出供应商响应或异常正文。
- 调度器强制执行全局 deadline、最大并发、最大 AI 请求数和输入大小预算；取消或降级必须保留原因码。
- 通知和评论失败均重试 2 次、默认不阻断，并在最终 CI 投递状态中写入脱敏状态和总尝试次数。
- 每次运行生成 `runId`，串联分析、验证、策略与投递日志；失败日志复用同一 ID，且诊断文本在输出前再次脱敏；不记录完整 diff。
...
当前实现已经完成原始/安全输入分级、候选项的 diff 锚定、行范围与证据一致性校验、分析器注册与预算调度，以及来源受控的确定性验证接线。首个
TypeScript AST 分析器只解析原始已提交 diff 的新增 `.ts`/`.tsx` 行，不读取工作区文件；它只对 AST 可直接证明的 `eval(...)`
调用产生 `ast` 验证证据并通过确定性链提升为 `verified`，不会提升 AI 发现。当前 TypeScript 7 尚未提供稳定编译器 API，因此
AST 适配器使用官方 `@typescript/typescript6` 兼容 API，与 TS7 构建并存。版本化
...
AI 预算；认证或结构化响应错误不重试。GitHub Actions 自动化分析器只从本地 Git `HEAD` 读取本次修改的工作流，解析为平台无关
自动化 IR 后以观察模式报告可变引用、最小权限与信任边界建议；它不执行配置，也不参与门禁。`DeepSeek`、Java AST 与未证明的 SARIF
仍只会产生 `anchored` 发现；已接入的本地
...
当前影响索引从已提交 TypeScript/Java diff 的变更声明建立 `SymbolIdentity`，使用限定名、参数签名和受控源码摘要保守关联 base/head
间的重命名、移动、重载变化与实现替换；多个候选或无法匹配时不会强行合并。索引同时提取新增静态 import、CommonJS require、调用、
实现/继承、配置、事件和持久化写入边，生成不含源码正文的 `ImpactPackage` 供 AI 解释已知关联。Git 基础设施适配器先把用户提供的 ref
验证为 commit，三点比较解析实际 merge base，再以文件数、单文件大小、总字符数和并发上限读取对象数据库中的 base/head 源码；它不读取
...
初始化可约束实例接收者；字符串、数字、布尔、`null` 字面量及明确基础类型注解/常量初始化的局部标识符可进一步筛选同参数数量的重载。
...
8. 在验证体系稳定后，再评估 AST 验证、受限测试执行、GitLab、更多模型和行级评论。首个受限 TypeScript AST
