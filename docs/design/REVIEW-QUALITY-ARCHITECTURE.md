...
实现/继承、配置、事件和持久化写入边，生成不含源码正文的 `ImpactPackage` 供 AI 解释已知关联。当前调用目标只来自已变更行并保持
`partial`/`unknown`，不是完整仓库调用图、控制流或行为覆盖证明。动态依赖、动态分派、反射、代码生成、未支持语言、无法锚定或索引失败
均明确保留为限制，不能表示无影响，也不能单独构成回归、缺失测试或门禁结论。
